...
// sent messages
// userInfo Messages
router.get("/infoMessageSent", async(req, res, next) => {
    await User.findById({_id: req.query.userId})
.then(user => {
    console.log('user ni ni', user)
    BlockSkalar.find({blockedUsername:user.username})
...
console.log('blocked', blocked)
            ...
            {username: user.username}
                ...
                if(allMsgs.length == nonyaOnce.length){ 
                    let allMsgsReverse = allMsgs;
                    console.log('sooner syd 277', allMsgsReverse );
                    allMsgsReverse.sort((a,b) => {
                        let newest = new Date(a.time),
                            older = new Date(b.time);
                            return newest - older
                    ...
                    console.log('final jess 2', allMsgsReverse)
                res.status(200).json({
                    message: 'Info messages sent fetched succesfully!',
                       messages: allMsgsReverse
                    });
                }
                  })
                ...
                return res.status(200).json({
                    message: "No sent messages to retrieve",
                    messages: documents
